...
public class PITPlayerModel {
    ...
    // keep track of incoming channels
    private HashSet<Integer> channel = new HashSet();

    // record all the markers in the state
    ...
    /**
     * Add card into snapshot
     * @param source source of card
     * @param card card to be added to the screenshot
     */
    private void addToSnapshot(int source, String card) {
        // do not count if the marker has not yet taken part in the snapshot
        if (state == null) {
            return;
        }

        // do not count if the source has been visited by a marker
        if (channel.contains(source)) {
            return;
        }

        // add value according to key and update state
    ...
    private void doMarker(Marker marker) throws Exception {
        ...
        // If the marker has not yet taken part in the snapshot
        if (state == null) {
            ...
            // record the state
            // Add each commodity string and the number of that commodity in hand into state.
            for (Object o : cards) {
                String card = (String) o;
                addToSnapshot(source, card);
            }
            System.out.println(toString(state));

            // do marker sending rule
            // for each outgoint channel c
            for (int i = 0; i < 5; i++) {
            ...
            if (source != -1) { // donot add to channel if it's from outer source
                channel.add(source);
            }
        } else if (!channel.contains(source)) {
            channel.add(source);

            // send out state if all channels have been visited by markers
            if (channel.size() == 4) {
    ...
    private void doReceiveTenderOffer(TenderOffer trade) throws Exception {
        ...
        if (Math.random() < 0.8) {
            ...
            addToSnapshot(trade.sourcePlayer, trade.tradeCard);
